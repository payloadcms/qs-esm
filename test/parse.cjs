...
test('parse()', async function (t) {
  ...
  t.test('decode dot keys correctly', function (st) {
    ...
    st.deepEqual(
      qs.parse('name.obj.subobject.first.godly.name=John&name.obj.subobject.last=Doe', {
        allowDots: true,
        decodeDotInKeys: false,
      }),
  ...
  t.test(
    'should decode dot in key of object, and allow enabling dot notation when decodeDotInKeys is set to true and allowDots is undefined',
    function (st) {
      st.deepEqual(
        qs.parse(
          'name%252Eobj%252Esubobject.first%252Egodly%252Ename=John&name%252Eobj%252Esubobject.last=Doe',
  ...
  t.test(
    'object-valued input with own `__proto__` does not mutate sub-object [[Prototype]]',
    function (st) {
      ...
      st.equal(
        Object.getPrototypeOf(out.user),
        Object.prototype,
        'sub-object [[Prototype]] is unchanged',
      )
...
test('parses empty keys', async function (t) {
  var qs = await import('../lib/index.js')
  var utils = await import('../lib/utils.js')
...
test('`duplicates` option', async function (t) {
  var qs = await import('../lib/index.js')
  var utils = await import('../lib/utils.js')
