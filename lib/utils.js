...
export const markOverflow = function markOverflow(obj, maxIndex) {
...
export const merge = function merge(target, source, options) {
  ...
  return Object.keys(source).reduce(function (acc, key) {
    ...
    if (has.call(acc, key)) {
      value = merge(acc[key], value, options)
    }
...
export const encode = function encode(str, _defaultEncoder, _kind, format) {
  ...
  for (let j = 0; j < string.length; j += limit) {
    let segment = string.length >= limit ? string.slice(j, j + limit) : string
