...
test('stringify()', async function (t) {
  var qs = await import('../lib/index.js')
  ...
  t.test('should encode the key of an empty array when allowEmptyArrays is set', function (st) {
    ...
    st.ok(
      'a b' in
        qs.parse(qs.stringify({ 'a b': [] }, { allowEmptyArrays: true }), {
  ...
  t.test('stringifies an array value with one item vs multiple items', function (st) {
    st.test('non-array item', function (s2t) {
      ...
      s2t.equal(
        qs.stringify({ a: 'c' }, { encodeValuesOnly: true, arrayFormat: 'brackets' }),
        'a=c',
      )
    ...
    st.test('array with a single item', function (s2t) {
      s2t.equal(
        qs.stringify({ a: ['c'] }, { encodeValuesOnly: true, arrayFormat: 'indices' }),
        'a[0]=c',
      )
      s2t.equal(
        qs.stringify({ a: ['c'] }, { encodeValuesOnly: true, arrayFormat: 'brackets' }),
    ...
    st.test('array with multiple items', function (s2t) {
      s2t.equal(
        qs.stringify({ a: ['c', 'd'] }, { encodeValuesOnly: true, arrayFormat: 'indices' }),
        'a[0]=c&a[1]=d',
      )
      s2t.equal(
        qs.stringify({ a: ['c', 'd'] }, { encodeValuesOnly: true, arrayFormat: 'brackets' }),
        'a[]=c&a[]=d',
      )
      s2t.equal(
        qs.stringify({ a: ['c', 'd'] }, { encodeValuesOnly: true, arrayFormat: 'comma' }),
        'a=c,d',
      )
      s2t.equal(
        qs.stringify(
    ...
    st.test('array with multiple items with a comma inside', function (s2t) {
      s2t.equal(
        qs.stringify({ a: ['c,d', 'e'] }, { encodeValuesOnly: true, arrayFormat: 'comma' }),
        'a=c%2Cd,e',
      )
      ...
      s2t.equal(
        qs.stringify({ a: ['c,d', 'e'] }, { arrayFormat: 'comma', commaRoundTrip: true }),
        'a=c%2Cd%2Ce',
      )
  ...
  t.test('stringifies a nested array value', function (st) {
    st.equal(
      qs.stringify({ a: { b: ['c', 'd'] } }, { encodeValuesOnly: true, arrayFormat: 'indices' }),
      'a[b][0]=c&a[b][1]=d',
    )
    st.equal(
      qs.stringify({ a: { b: ['c', 'd'] } }, { encodeValuesOnly: true, arrayFormat: 'brackets' }),
      'a[b][]=c&a[b][]=d',
    )
    st.equal(
      qs.stringify({ a: { b: ['c', 'd'] } }, { encodeValuesOnly: true, arrayFormat: 'comma' }),
      'a[b]=c,d',
    )
    st.equal(
  ...
  t.test('stringifies comma and empty array values', function (st) {
    st.equal(
      qs.stringify({ a: [',', '', 'c,d%'] }, { encode: false, arrayFormat: 'indices' }),
      'a[0]=,&a[1]=&a[2]=c,d%',
    )
    st.equal(
      qs.stringify({ a: [',', '', 'c,d%'] }, { encode: false, arrayFormat: 'brackets' }),
      'a[]=,&a[]=&a[]=c,d%',
    )
    st.equal(
      qs.stringify({ a: [',', '', 'c,d%'] }, { encode: false, arrayFormat: 'comma' }),
      'a=,,,c,d%',
    )
    st.equal(
      qs.stringify({ a: [',', '', 'c,d%'] }, { encode: false, arrayFormat: 'repeat' }),
      'a=,&a=&a=c,d%',
    )
  ...
  t.test('stringifies comma and empty non-array values', function (st) {
    st.equal(
      qs.stringify({ a: ',', b: '', c: 'c,d%' }, { encode: false, arrayFormat: 'indices' }),
      'a=,&b=&c=c,d%',
    )
    st.equal(
      qs.stringify({ a: ',', b: '', c: 'c,d%' }, { encode: false, arrayFormat: 'brackets' }),
      'a=,&b=&c=c,d%',
    )
    st.equal(
      qs.stringify({ a: ',', b: '', c: 'c,d%' }, { encode: false, arrayFormat: 'comma' }),
      'a=,&b=&c=c,d%',
    )
    st.equal(
      qs.stringify({ a: ',', b: '', c: 'c,d%' }, { encode: false, arrayFormat: 'repeat' }),
      'a=,&b=&c=c,d%',
    )
  ...
  t.test('stringifies a nested array value with dots notation', function (st) {
    ...
    st.equal(
      qs.stringify({ a: { b: ['c', 'd'] } }, { allowDots: true, encodeValuesOnly: true }),
      'a.b[0]=c&a.b[1]=d',
  ...
  t.test('stringifies an empty array in different arrayFormat', function (st) {
    ...
    // arrayFormat default
    st.equal(
      qs.stringify({ a: [], b: [null], c: 'c' }, { encode: false, arrayFormat: 'indices' }),
      'b[0]=&c=c',
    )
    st.equal(
      qs.stringify({ a: [], b: [null], c: 'c' }, { encode: false, arrayFormat: 'brackets' }),
      'b[]=&c=c',
    )
    st.equal(
      qs.stringify({ a: [], b: [null], c: 'c' }, { encode: false, arrayFormat: 'repeat' }),
      'b=&c=c',
    )
    st.equal(
      qs.stringify({ a: [], b: [null], c: 'c' }, { encode: false, arrayFormat: 'comma' }),
      'b=&c=c',
    )
    st.equal(
      qs.stringify(
  ...
  t.test(
    'does not crash on null/undefined entries in arrayFormat=comma with encodeValuesOnly',
    function (st) {
      ...
      st.equal(
        qs.stringify(
          { a: [null] },
          { arrayFormat: 'comma', encodeValuesOnly: true, strictNullHandling: true },
        ),
        'a',
  ...
  t.test('encodeValuesOnly', function (st) {
    ...
    st.equal(
      qs.stringify({ a: 'b', c: ['d', 'e'], f: [['g'], ['h']] }, { arrayFormat: 'indices' }),
      'a=b&c%5B0%5D=d&c%5B1%5D=e&f%5B0%5D%5B0%5D=g&f%5B1%5D%5B0%5D=h',
      'no encodeValuesOnly + indices',
    )
    st.equal(
      qs.stringify({ a: 'b', c: ['d', 'e'], f: [['g'], ['h']] }, { arrayFormat: 'brackets' }),
      'a=b&c%5B%5D=d&c%5B%5D=e&f%5B%5D%5B%5D=g&f%5B%5D%5B%5D=h',
      'no encodeValuesOnly + brackets',
    )
    st.equal(
      qs.stringify({ a: 'b', c: ['d', 'e'], f: [['g'], ['h']] }, { arrayFormat: 'repeat' }),
      'a=b&c=d&c=e&f=g&f=h',
  ...
  t.test('stringifies sparse arrays', function (st) {
    ...
    st.equal(
      qs.stringify({ a: [, '2', , , '1'] }, { encodeValuesOnly: true, arrayFormat: 'indices' }),
      'a[1]=2&a[4]=1',
    )
    st.equal(
      qs.stringify({ a: [, '2', , , '1'] }, { encodeValuesOnly: true, arrayFormat: 'brackets' }),
      'a[]=2&a[]=1',
    )
    st.equal(
      qs.stringify({ a: [, '2', , , '1'] }, { encodeValuesOnly: true, arrayFormat: 'repeat' }),
      'a=2&a=1',
    )
...
test('stringifies empty keys', async function (t) {
  var qs = await import('../lib/index.js')
  var utils = await import('../lib/utils.js')
  ...
  t.test('edge case with object/arrays', function (st) {
    ...
    st.deepEqual(
      qs.stringify({ '': { '': [2, 3] } }, { encode: false, arrayFormat: 'indices' }),
      '[][0]=2&[][1]=3',
    )
    st.deepEqual(
      qs.stringify({ '': { '': [2, 3], a: 2 } }, { encode: false, arrayFormat: 'indices' }),
      '[][0]=2&[][1]=3&[a]=2',
    )
